{ ... }:
let
  mykey = "ssh-rsa  openpgp:0x79E90D11";
  orbstack = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKAVHHCqe377IiEZFloiF7jKl7JCex2tj9MKVDzsAr7s orbstack";
  mac = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIf0Z1OuazFhhbZgA2s2jnFgeIx/KAXZJO+SziK5neFn mac";
  phonekey = "ssh-rsa  u0_a401@localhost";
in
{
  # Enable the OpenSSH daemon.
  services.openssh = {
    enable = true;
    settings = {
      PasswordAuthentication = false;
      KbdInteractiveAuthentication = false;
    };
  };

  users.users.pengu.openssh.authorizedKeys.keys = [
    mac
    mykey
    orbstack
    phonekey
  ];
  users.users.root.openssh.authorizedKeys.keys = [
    mykey
  ];
}
